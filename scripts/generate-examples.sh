#!/bin/bash
# https://gist.github.com/mohanpedala/1e2ff5661761d3abd0385e8223e16425
set -e -x -v -u -o pipefail

SCRIPT_DIR=$(realpath "$(dirname "${BASH_SOURCE[0]}")")
source "${SCRIPT_DIR}/utilities/common.sh"

# NOTE: Use dev requirements to generate the README because the README uses
# shell() with some tools that we only want to install into dev environment.
VENV_PATH="${PWD}/.cache/scripts/.venv" source "${PROJ_PATH}/scripts/utilities/ensure-venv.sh"
TOML=${PROJ_PATH}/pyproject.toml EXTRA=dev \
  DEV_VENV_PATH="${PWD}/.cache/scripts/.venv" \
  TARGET_VENV_PATH="${PWD}/.cache/scripts/.venv" \
  bash "${PROJ_PATH}/scripts/utilities/ensure-reqs.sh"



CUSTOM_WARNING=$(cat << 'EOF'
#!/bin/bash
# WARNING: This file is auto-generated by snipinator. Do not edit directly.
# SOURCE: `{template_file_name}`.

.
EOF
)

python -m snipinator.cli \
  -t "${PROJ_PATH}/comfy_fulcrum/cli/examples/cli_example.sh.jinja2" \
  --args '{"example_type": "python_module", "module_name": "comfy_fulcrum.cli"}' \
  --warning-header "${CUSTOM_WARNING%.}" \
  --rm \
  --force \
  --create \
  -o "${PROJ_PATH}/comfy_fulcrum/cli/examples/cli_example.sh" \
  --chmod-ro \
  --skip-unchanged


python -m snipinator.cli \
  -t "${PROJ_PATH}/comfy_fulcrum/cli/examples/cli_example.sh.jinja2" \
  --args '{"example_type": "docker-local", "docker_image_name": "my-comfy_fulcrum-image"}' \
  --warning-header "${CUSTOM_WARNING%.}" \
  --rm \
  --force \
  --create \
  -o "${PROJ_PATH}/comfy_fulcrum/cli/examples/cli_example-local-docker_example.sh" \
  --chmod-ro \
  --skip-unchanged


LAST_VERSION=$(tomlq -r -e '.["tool"]["comfy_fulcrum-project-metadata"]["last_stable_release"]' pyproject.toml)
python -m snipinator.cli \
  -t "${PROJ_PATH}/comfy_fulcrum/cli/examples/cli_example.sh.jinja2" \
  --args '{"example_type": "docker-remote", "docker_image_name": "ghcr.io/realazthat/comfy_fulcrum:v'"${LAST_VERSION}"'"}' \
  --warning-header "${CUSTOM_WARNING%.}" \
  --rm \
  --force \
  --create \
  -o "${PROJ_PATH}/comfy_fulcrum/cli/examples/cli_example-remote-docker_example-noautorun.sh" \
  --chmod-ro \
  --skip-unchanged
